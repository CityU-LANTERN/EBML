...
from .attention_layer import SMAB_small


#############################
#  film adaptation network  #
#############################
...
class FilmLayerNetwork(nn.Module):
    ...
    def __init__(self, num_maps, num_blocks, z_g_dim):
        ...
        #     self.gamma1_regularizers.append(
        #         torch.nn.Parameter(torch.nn.init.normal_(torch.empty(1, num_maps), 0, 0.001),
        #                            requires_grad=True))
        #
        #     self.beta1_processors.append(self._make_layer(num_maps))
        #     self.beta1_regularizers.append(torch.nn.Parameter(torch.nn.init.normal_(torch.empty(1, num_maps), 0, 0.001),
        #                                                       requires_grad=True))
        #
        #     self.gamma2_processors.append(self._make_layer(num_maps))
        #     self.gamma2_regularizers.append(
        #         torch.nn.Parameter(torch.nn.init.normal_(torch.empty(1, num_maps), 0, 0.001),
        #                            requires_grad=True))
        #
        #     self.beta2_processors.append(self._make_layer(num_maps))
        #     self.beta2_regularizers.append(torch.nn.Parameter(torch.nn.init.normal_(torch.empty(1, num_maps), 0, 0.001),
        #                            requires_grad=True))
    ...
    def forward_no_adaptation(self, x):
        """
        Dummy forward pass, output gamma(s)=1 and beta(s）=0 which corresponds to no film adaptation.
        :param x: (torch.tensor) dummy variable (task level representation z).
        :return: (list::dictionaries) Dictionary for every block in layer. Dictionary contains all the parameters
                 necessary to adapt layer in base network. Base network is aware of dict structure and can pull params
                 out during forward pass.
        """
        num_phi_samples = x.shape[0]
        block_params = []
        for block in range(self.num_blocks):
            block_param_dict = {
                'gamma1': torch.ones_like(self.gamma1_regularizers[block]),
                'beta1': torch.zeros_like(self.beta1_regularizers[block]),
                'gamma2': torch.ones_like(self.gamma2_regularizers[block]),
                'beta2': torch.zeros_like(self.beta2_regularizers[block]),
            }
            block_params.append(block_param_dict)
        return block_params
...
class FilmAdaptationNetwork(nn.Module):
    ...
    def forward(self, x, no_film=False):
        """
        ...
        """
        if no_film:
            return [self.layers[layer].forward_no_adaptation(x) for layer in range(self.num_target_layers)]
        else:
...
class MultiDomainFilmLayerNetwork(nn.Module):
    """
    Single adaptation network for generating the parameters of each layer in the base network. Will be wrapped around
    by FilmAdaptationNetwork.
    :param num_maps: (int) Number of output maps to be adapted in base network layer.
    ...
    def __init__(self, num_maps, num_blocks, z_g_dim):
        ...
        for _ in range(self.num_blocks):
            ...
            self.gamma1_list.append(
                torch.nn.Parameter(torch.nn.init.normal_(torch.empty(num_maps, 8), 0, 0.2), requires_grad=True))
            self.gamma2_list.append(
                torch.nn.Parameter(torch.nn.init.normal_(torch.empty(num_maps, 8), 0, 0.1), requires_grad=True))
            self.beta1_list.append(
                torch.nn.Parameter(torch.nn.init.normal_(torch.empty(num_maps, 8), 0, 0.2), requires_grad=True))
            self.beta2_list.append(
                torch.nn.Parameter(torch.nn.init.normal_(torch.empty(num_maps, 8), 0, 0.1), requires_grad=True))

            ## Decoupled experts, no SAB applied
            self.gamma1De_list.append(
                torch.nn.Parameter(torch.nn.init.normal_(torch.empty(num_maps, 8), 0, 0.01), requires_grad=True))
            self.gamma2De_list.append(
                torch.nn.Parameter(torch.nn.init.normal_(torch.empty(num_maps, 8), 0, 0.01), requires_grad=True))
            self.beta1De_list.append(
                torch.nn.Parameter(torch.nn.init.normal_(torch.empty(num_maps, 8), 0, 0.01), requires_grad=True))
            self.beta2De_list.append(
                torch.nn.Parameter(torch.nn.init.normal_(torch.empty(num_maps, 8), 0, 0.01), requires_grad=True))
    ...
    def _make_layer(size):
        ...
        #    SAB(size, size, num_heads=int(size/16), ln=False)
        # )
        return SMAB_small(size, min(96, size), min(96, size), min(96, size), size, num_heads1=min(size // 32, 3),
                          num_heads2=size // 32, ln=False)
    ...
    def forward(self, gate, x, mode='train'):
        ...
        for block in range(self.num_blocks):
            ...
            if mode == 'test':
                gamma1Trans, softG11, softG12 = self.gamma1_processors[block](QG1, gamma1,
                                                                              'test')  # test mode, return softmax
                beta1Trans, softB11, softB12 = self.beta1_processors[block](QB1, beta1,
                                                                            'test')  # test mode, return softmax
                gamma2Trans, softG21, softG22 = self.gamma2_processors[block](QG2, gamma2,
                                                                              'test')  # test mode, return softmax
                beta2Trans, softB21, softB22 = self.beta2_processors[block](QB2, beta2,
                                                                            'test')  # test mode, return softmax

                ## save softmax transformer params
                softmaxTrans[block][0] = torch.cat([softG11.mean(0).view(8, 8), softG12.mean(0).view(1, 8)],
                                                   dim=0)  # 9x8
                softmaxTrans[block][1] = torch.cat([softB11.mean(0).view(8, 8), softB12.mean(0).view(1, 8)],
                                                   dim=0)  # 9x8
                softmaxTrans[block][2] = torch.cat([softG21.mean(0).view(8, 8), softG22.mean(0).view(1, 8)],
                                                   dim=0)  # 9x8
                softmaxTrans[block][3] = torch.cat([softB21.mean(0).view(8, 8), softB22.mean(0).view(1, 8)],
                                                   dim=0)  # 9x8
            ...
            block_param_dict = {
                'gamma1': (alpha * (gamma1Trans.view(-1)) + (1 - alpha) * ((gamma1De * gate).sum(1))) *
                          self.gamma1_regularizers[block] +
                          torch.ones_like(self.gamma1_regularizers[block]),
                'beta1': (alpha * (beta1Trans.view(-1)) + (1 - alpha) * ((beta1De * gate).sum(1))) *
                         self.beta1_regularizers[block],
                'gamma2': (alpha * (gamma2Trans.view(-1)) + (1 - alpha) * ((gamma2De * gate).sum(1))) *
                          self.gamma2_regularizers[block] +
                          torch.ones_like(self.gamma2_regularizers[block]),
                'beta2': (alpha * (beta2Trans.view(-1)) + (1 - alpha) * ((beta2De * gate).sum(1))) *
                         self.beta2_regularizers[block]
            }
            block_params.append(block_param_dict)
...
class MultiDomainFilmAdaptationNetwork(nn.Module):
    """
    FiLM adaptation network (outputs FiLM adaptation parameters for all layers in a base feature extractor).
    :param layer: (FilmLayerNetwork) Layer object to be used for adaptation.
    :param num_maps_per_layer: (list::int) Number of feature maps for each layer in the network.
