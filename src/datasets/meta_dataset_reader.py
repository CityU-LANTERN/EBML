...
class MetaDatasetReader:
    ...
    """
    def __init__(self, data_path, mode, train_set, validation_set, test_set, max_way_train, max_way_test, max_support_train, max_support_test, way=None,shot=None,query_train=None,query_test=None,oneshot=False):
        ...
        print(f'{f"":=^50}')
        print(f"Config to 5-way 1-shot Dataloader") if oneshot else print(f"Config to varying-way varying-shot Dataloader")
        print(f'{f"":=^50}')
        # tf.compat.v1.disable_eager_execution()
        # self.session = tf.compat.v1.Session()
        gin.parse_config_file(os.path.dirname(os.path.abspath(__file__))+'/meta_dataset_config.gin') #todo: change this relative import

        if mode == 'train' or mode == 'train_test':
            train_episode_description = self._get_train_episode_description(max_way_train, max_support_train,way,shot,query_train)
            ...
            test_episode_description = self._get_test_episode_description(max_way_test, max_support_test,way,shot,query_test)
    ...
    def _get_task(self, next_task,session):
        ...
        task_dict = {
            ...
            'context_source_labels':episode[2],
            'target_source_labels':episode[5],
            'domain': source_id
            }
        return task_dict
    ...
    def _get_train_episode_description(self, max_way_train, max_support_train,num_ways,num_support,num_query):
        return config.EpisodeDescriptionConfig(
            num_ways=num_ways,
            num_support=num_support,
            num_query=num_query,
            min_ways=5,
            max_ways_upper_bound= 5 if self.oneshot else max_way_train,
            max_num_query=10,
            max_support_set_size=5 if self.oneshot else max_support_train,
            max_support_size_contrib_per_class=1 if self.oneshot else 100,
    ...
    def _get_test_episode_description(self, max_way_test, max_support_test,num_ways,num_support,num_query):
        return config.EpisodeDescriptionConfig(
            num_ways=num_ways,
            num_support=num_support,
            num_query=num_query,
            min_ways=5,
            max_ways_upper_bound=5 if self.oneshot else max_way_test,
            max_num_query=10,
            max_support_set_size=5 if self.oneshot else max_support_test,
            max_support_size_contrib_per_class=1 if self.oneshot else 100,
            min_log_weight=-0.69314718055994529, # np.cnaps_layer_log.txt(0.5)
            max_log_weight=0.69314718055994529, # np.cnaps_layer_log.txt(2)
            ignore_dag_ontology=False,
...
class SingleDatasetReader:
    ...
    def __init__(self, data_path, mode, dataset, way, shot, query_train, query_test):
        ...
        tf.compat.v1.disable_eager_execution()
        self.session = tf.compat.v1.Session()
        gin.parse_config_file(os.path.dirname(os.path.abspath(__file__))+'/meta_dataset_config.gin')
